...
export default function SuperAdminHospitalsPage() {
  ...
  const handleCreateAdmin = async () => {
    ...
    setCreateError('');
    try {
      if (!selectedHospital?.id) throw new Error('Hospital is required');
      ...
      setCreateError(e instanceof Error ? e.message : mode === 'create' ? 'Failed to create admin' : 'Failed to update admin');
    } finally {
      ...
      <SuperAdminLayout>
        <div className="flex items-start justify-between gap-6 mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-1">Select a hospital and create admin credentials</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border p-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
            <div className="flex-1">
              <input
                ...
                placeholder="Search hospitals by name / district / province"
          ...
          {loading ? (
            <div className="py-10 text-center text-sm text-gray-600">Loading hospitals...</div>
          ) : filtered.length === 0 ? (
            <div className="py-10 text-center">
              <p className="text-sm font-medium text-gray-900">No hospitals found</p>
              <p className="text-xs text-gray-600 mt-1">Try a different search or add a new hospital.</p>
            </div>
          ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="text-left py-3 text-xs font-semibold text-gray-700">District</th>
                    <th className="text-left py-3 text-xs font-semibold text-gray-700">Province</th>
                    <th className="text-left py-3 text-xs font-semibold text-gray-700">Phone</th>
                    <th className="text-right py-3 text-xs font-semibold text-gray-700">Action</th>
                  </tr>
                </thead>
                <tbody>
                  {filtered.map((h) => (
                    <tr key={h.id} className="border-b last:border-0 hover:bg-gray-50">
                      ...
                      <td className="py-4 text-sm text-gray-700">{h.district ?? '—'}</td>
                      <td className="py-4 text-sm text-gray-700">{h.province ?? '—'}</td>
                      <td className="py-4 text-sm text-gray-700">{h.phone ?? '—'}</td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="p-6 border-b border-gray-200 flex items-center justify-between">
                <div>
                  <h2 className="text-xl font-bold text-gray-900">
                    {mode === 'create' ? 'Create Hospital Admin Credentials' : 'Edit Hospital Admin'}
                  </h2>
                  <p className="text-sm text-gray-600 mt-1">
                    Hospital: <span className="font-semibold text-gray-900">{selectedHospital?.name ?? '—'}</span>
                  </p>
                </div>
                <button
              ...
              <div className="p-6 border-t border-gray-200 flex justify-end gap-3">
                ...
                >
                  {createLoading ? (mode === 'create' ? 'Creating...' : 'Saving...') : (mode === 'create' ? 'Create Admin' : 'Save Changes')}
                </button>
              </div>
            </div>
          </div>
